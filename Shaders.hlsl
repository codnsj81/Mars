...
//
//[maxvertexcount(4)]
//void GS(point VS_OUT input[1], uint primID : SV_PrimitiveID, inout TriangleStram <GS_OUT> outStream)
//{
//	float3 f3Look = normalize(gvCameraPosition - input.instancePosition);
//	float3 f3Up = float3(0.0f, 1.0f, 0.0f);
//	float3 f3Right = normalize(cross(f3Up, f3Look));
...
//
//	float4 pVertices[4];
//	pVertices[0] = float4(input[0], centerW + fHalfW * cRight - fHalfH * vUp, 1.0f);
//	pVertices[1] = float4(input[0], centerW + fHalfW * cRight + fHalfH * vUp, 1.0f);
//	pVertices[2] = float4(input[0], centerW - fHalfW * cRight - fHalfH * vUp, 1.0f);
//	pVertices[3] = float4(input[0], centerW - fHalfW * cRight + fHalfH * vUp, 1.0f);
