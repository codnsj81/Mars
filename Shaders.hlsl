...
VS_BILLBOARD_INSTANCING_OUTPUT VSBillboardInstancing(VS_BILLBOARD_INSTANCING_INPUT input)
{
	...
	output.position = float4(input.instancePosition,1);
...
[maxvertexcount(4)]
void GSBillboardInstancing(point VS_BILLBOARD_INSTANCING_OUTPUT input[1],  inout TriangleStream <GS_OUT> outStream)
{
	float3 pos = input[0].position.xyz;
	float3 vLook = normalize(gvCameraPosition - pos);
	float3 vUp = float3(0.0f, 1.0f, 0.0f);
	float3 vRight = normalize(cross(vUp, vLook));

	float fHalfW = input[0].billboardInfo.x * 0.5f;
	float fHalfH = input[0].billboardInfo.y * 0.5f;

	float4 pVertices[4];
	pVertices[0] = float4(pos + fHalfW * vRight - fHalfH * vUp, 1.0f);
	pVertices[1] = float4(pos + fHalfW * vRight + fHalfH * vUp, 1.0f);
	pVertices[2] = float4(pos - fHalfW * vRight - fHalfH * vUp, 1.0f);
	pVertices[3] = float4(pos - fHalfW * vRight + fHalfH * vUp, 1.0f);
