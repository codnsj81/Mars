...
void CBillboardObjectsShader::BuildObjects(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, void* pContext)
{
	...
	ppGrassTextures->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Tree.dds", 0);

	m_pBillboardMaterial = new CMaterial();
	m_pBillboardMaterial->SetTexture( ppGrassTextures);
...
void CBillboardObjectsShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState * [m_nPipelineStates];
...
D3D12_INPUT_LAYOUT_DESC CBillboardObjectsShader::CreateInputLayout()
{
	...
	pd3dInputElementDescs[1] = { "BILLBOARDINFO", 0, DXGI_FORMAT_R32G32_FLOAT, 0, 12, D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA, 1 };
...
D3D12_INPUT_LAYOUT_DESC CDiffuseVertexShader::CreateInputLayout()
{
	UINT nInputElementDescs = 2;
	D3D12_INPUT_ELEMENT_DESC* pd3dInputElementDescs = new D3D12_INPUT_ELEMENT_DESC[nInputElementDescs];
...
D3D12_SHADER_BYTECODE CDiffuseVertexShader::CreateVertexShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSDiffused", "vs_5_1", &m_pd3dVertexShaderBlob));
}

D3D12_SHADER_BYTECODE CDiffuseVertexShader::CreatePixelShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "PSDiffused", "ps_5_1", &m_pd3dPixelShaderBlob));
}

void CDiffuseVertexShader::CreateShader(ID3D12Device* pd3dDevice, ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets)
{
...
void CUIShader::CreateShader(ID3D12Device* pd3dDevice,  ID3D12RootSignature* pd3dGraphicsRootSignature, UINT nRenderTargets)
{
	m_nPipelineStates = 1;
	m_ppd3dPipelineStates = new ID3D12PipelineState *[m_nPipelineStates];
...
void CPostProcessingShader::CreateGraphicsRootSignature(ID3D12Device* pd3dDevice)
{
	...
	pd3dDescriptorRanges[0].BaseShaderRegister = 6; //Texture[]
...
void CPostProcessingShader::CreateShaderVariables(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList)
{
	...
	UINT ncbElementBytes = sizeof(int); //256의 배수
...
D3D12_SHADER_BYTECODE CPostProcessingByTestShader::CreateVertexShader()
{
	return(CShader::CompileShaderFromFile(L"Shaders.hlsl", "VSPostProcessing", "vs_5_1", &m_pd3dVertexShaderBlob));

}

D3D12_SHADER_BYTECODE CPostProcessingByTestShader::CreatePixelShader()
{
