...
CGameFramework::CGameFramework()
{
	...
	_tcscpy_s(m_pszFrameRate, _T("2015184013 ("));
}
...
void CGameFramework::CreateSwapChainRenderTargetViews()
{
	...
	m_pd3dRenderResourceBuffer = pTexture->CreateTexture(m_pd3dDevice, m_pd3dCommandList, m_nWndClientWidth, m_nWndClientHeight, DXGI_FORMAT_R8G8B8A8_UNORM, D3D12_RESOURCE_FLAG_NONE, D3D12_RESOURCE_STATE_COMMON, NULL, 0);
	m_pd3dRenderResourceBuffer->AddRef();
...
void CGameFramework::ChangeSwapChainState()
{
	...
	if (m_pPostProcessingByTestShader) m_pPostProcessingByTestShader->Release();
	if (m_pd3dRenderResourceBuffer) m_pd3dRenderResourceBuffer->Release();
...
{
	if (m_pScene[0]) m_pScene[0]->OnProcessingMouseMessage(hWnd, nMessageID, wParam, lParam);
	switch (nMessageID)
	{
		case WM_LBUTTONDOWN:
		case WM_RBUTTONDOWN:
...
{
	if (m_pScene) m_pScene[0]->OnProcessingKeyboardMessage(hWnd, nMessageID, wParam, lParam);
	switch (nMessageID)
	{
		case WM_KEYUP:
			switch (wParam)
...
void CGameFramework::BuildObjects()
{
	...
	if (m_pScene)
	{
		m_pScene[1]->ReleaseUploadBuffers();
		m_pScene[0]->ReleaseUploadBuffers();
	}
...
void CGameFramework::ReleaseObjects()
{
	...
	if (m_pScene[0])
		m_pScene[0]->ReleaseObjects();
	if (m_pScene[1])
		m_pScene[1]->ReleaseObjects();
...
void CGameFramework::ProcessInput()
{
	...
	if (m_nSceneNum == 0) return;

	if (GetKeyboardState(pKeysBuffer) && m_pScene) bProcessedByScene = m_pScene[m_nSceneNum]->ProcessInput(pKeysBuffer);
	if (!bProcessedByScene)
	{
		...
		DWORD dwRotation = 0;
		if (pKeysBuffer['A'] & 0xF0) dwRotation |= DIR_LEFT;
		if (pKeysBuffer['D'] & 0xF0) dwRotation |= DIR_RIGHT;

		if (pKeysBuffer[VK_SPACE] & 0xF0)
		{
			if (pKeysBuffer[VK_SHIFT] & 0xF0) dwDirection |= DIR_DOWN;
			else dwDirection |= DIR_UP;
		}
		...
		if (dwRotation != 0)
			m_pPlayer->Rotate(dwRotation, 3.f);
		else
			m_pPlayer->SetTurningState(false);
		...
		if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
		{
			if (cxDelta || cyDelta)
			{
				if (pKeysBuffer[VK_RBUTTON] & 0xF0)
					m_pPlayer->GetCamera()->Rotate(cyDelta, 0.0f, -cxDelta);
				else
					m_pPlayer->GetCamera()->Rotate(cyDelta, cxDelta, 0.0f);
			}
		...
		if (pKeysBuffer[VK_RBUTTON] & 0xF0)
		{
			if (m_bShoot)
			{
				dynamic_cast<CGameScene*>(m_pScene[1])->CreateBullet();
				m_bShoot = false;
			}
			else
			{
				m_fShootTime += m_GameTimer.GetTimeElapsed();
				if (m_fShootTime >= m_fShootSpeed)
				{
					m_bShoot = true;
					m_fShootTime = 0;
				}
...
void CGameFramework::AnimateObjects()
{
	...
	if (m_pScene[m_nSceneNum]) m_pScene[m_nSceneNum]->AnimateObjects(fTimeElapsed,m_pd3dCommandList);
...
{    
	...
	float pfClearColor[4] = { 0.0f, 0.125f, 0.125f, 1.0f };
	D3D12_CPU_DESCRIPTOR_HANDLE d3dDsvCPUDescriptorHandle = m_pd3dDsvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();

	m_pd3dCommandList->ClearRenderTargetView(m_pd3dRtvSwapChainBackBufferCPUHandles[m_nSwapChainBufferIndex], pfClearColor/*Colors::Azure*/, 0, NULL);
	...
	if (m_pScene[m_nSceneNum]) m_pScene[m_nSceneNum]->Render(m_pd3dCommandList, d3dDsvCPUDescriptorHandle, m_pCamera);
	...
	::SynchronizeResourceTransition(m_pd3dCommandList, m_pd3dRenderResourceBuffer, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COPY_DEST);

	m_pd3dCommandList->CopyResource(m_pd3dRenderResourceBuffer, m_ppd3dSwapChainBackBuffers[m_nSwapChainBufferIndex]);

	::SynchronizeResourceTransition(m_pd3dCommandList, m_pd3dRenderResourceBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_COMMON);
